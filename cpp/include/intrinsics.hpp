...
void destruct(const ContainerType<_Tp, Args...> &p) {
  static_assert(std::is_destructible_v<_Tp> && std::is_destructible_v<Args...>);
  for (const auto &v : p) {
    v.~_Tp();
  }
}
 ...
 *
 */
template <typename _Tp> using raw_ptr = _Tp *;
 ...
 */
template <partially_formed _Tp> eop::raw_ptr<_Tp> ptr_construct(_Tp &&x) {
